(function () {
  ...
  function getStageAging() {
    ...
    const readyAges = orders
      .filter(o => o.status === "Ready for Packing")
      .map(o => {
  ...
  function updateAccessByRole() {
    ...
    if (role === "picker") {
      ...
      const exNav = document.getElementById("adminExceptionNav");
      if (exNav) exNav.style.display = "block";
    } else {
      const exNav = document.getElementById("adminExceptionNav");
      if (exNav) exNav.style.display = "none";
    }
  }

  function showScreen(screenId) {
    const role = String((window.appState && window.appState.currentRole) || "").toLowerCase();
    if (role === "picker" && screenId !== "pickerLogin" && screenId !== "pickingWorkflow" && screenId !== "welcomeScreen") {
      screenId = "pickerLogin";
    }
    if (role === "packer" && screenId !== "packerDashboard" && screenId !== "welcomeScreen") {
      screenId = "packerDashboard";
    }
    if (role && role !== "admin" && screenId === "adminExceptionScreen") {
      screenId = role === "picker" ? "pickerLogin" : role === "packer" ? "packerDashboard" : "welcomeScreen";
    }
    ...
    if (screenId === "adminExceptionScreen" && typeof renderAdminExceptionScreen === "function") {
      setTimeout(renderAdminExceptionScreen, 50);
    }
  ...
  function renderReadyQueueTable() {
    const container = document.getElementById("readyQueueTable");
    if (!container) return;

    const rows = getReadyQueue();

    if (!rows.length) {
      container.innerHTML = "<p style='margin-top:12px;'>No orders currently waiting in ready queue.</p>";
      return;
    }
